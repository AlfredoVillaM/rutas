...
export class TvShowsService {
  ...
  // FORMULARIO BASADO EN TEMPLATES
  // public name: string = "";
  ...
  public deleteElement(name: string): void {
    this._shows = this._shows.filter(show => show.name != name);
  }

  // FORMULARIO NGMODEL SIMPLE
  // public createElement(): void {
  // FORMULARIO BASADO EN PLANTILLAS
  // public createElement(form: NgForm): void {
  // FORMULARIO REACTIVO
  // public createElement(): void {
  public createElement(show: Show,  selectedShow: Show | null): void {
    if (selectedShow) {
      const index = this._shows.findIndex(s => s.name === selectedShow?.name);
      if (index !== -1) {
        this._shows[index] = show;
      }
    } else {
  ...
  public sortByAlphabetic(): void {
    this._shows.sort((a, b) => {return a.name.localeCompare(b.name)});
  }
